...
const App = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/generateConversationId${window.location.pathname}`);
  ...
  const sendQuestion = async (e) => {
    ...
    if (question.toLowerCase().startsWith("where did")){
      try {
        const response = await axios.get(`http://localhost:8080/extractRecentPercentage/${conversationId}`);
      ...
      try {
        const response = await axios.post(`http://localhost:8080/askQuestion/${conversationId}`, {'question': question});
  ...
  return (
    <div className='flex flex-col justify-start items-center p-5 h-screen bg-black'>
      <div className='flex flex-row justify-around items-center p-5 my-5 rounded-lg w-full bg-gray-500'>
        {!getLoading && <ReactPlayer url={`http://localhost:8080/lectures${window.location.pathname}.mp4`} width="640px" height="350px" playing={true} muted={true} controls={true} ref={playerRef} />}
